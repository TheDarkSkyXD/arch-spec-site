...
async def seed_sample_implementation_prompts(db, clean_all: bool = False):
    ...
    try:
        ...
        if clean_all and count > 0:
            print("Clean all option enabled. Removing all existing sample implementation prompts...")
            logger.info("Clean all option enabled. Removing all existing sample implementation prompts...")
            ...
            logger.info(f"Deleted {delete_result.deleted_count} sample implementation prompts documents")
...
async def load_implementation_prompts_from_files() -> Dict[str, List[ImplementationPrompt]]:
    ...
    logger.info(f"Found {len(category_dirs)} category directories: {[d.name for d in category_dirs]}")
    ...
    for category_dir in category_dirs:
        ...
        prompt_files = sorted([f for f in category_dir.iterdir() if f.is_file() and f.suffix == '.txt'])
        print(f"  Category {category_name}: Found {len(prompt_files)} prompt files: {[f.name for f in prompt_files]}")
        logger.info(f"  Category {category_name}: Found {len(prompt_files)} prompt files: {[f.name for f in prompt_files]}")
        ...
        for prompt_file in prompt_files:
            ...
            try:
                ...
                logger.info(f"    Added prompt of type {prompt_type} with {len(content)} characters")
            except Exception as e:
                print(f"    Error processing file {prompt_file}: {str(e)}")
                logger.error(f"    Error processing file {prompt_file}: {str(e)}")
    
    print(f"Loaded a total of {sum(len(p) for p in prompts_data.values())} prompts across {len(prompts_data)} categories")
    logger.info(f"Loaded a total of {sum(len(p) for p in prompts_data.values())} prompts across {len(prompts_data)} categories")
    return prompts_data 