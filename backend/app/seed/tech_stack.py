...
TECH_STACK_DATA = {
    "techStackOptions": {
        ...
        # ... Rest of the tech stack data will be added in the database
    }
}

def check_compatibility(tech_choice: Dict[str, str]) -> Dict[str, Any]:
    """
    Check compatibility between selected technology choices.
    
    Args:
        tech_choice: Dict containing selected technologies
            Example: {
    ...
    if "frontend_framework" in tech_choice and "state_management" in tech_choice:
        frontend_framework = tech_choice["frontend_framework"]
        state_management = tech_choice["state_management"]
    ...
    if "backend_framework" in tech_choice and "database" in tech_choice:
        backend_framework = tech_choice["backend_framework"]
        database = tech_choice["database"]
        
        # Find the backend framework in the data
        backend_data = None
        for fw in TECH_STACK_DATA["techStackOptions"]["backend"]["frameworks"]:
    ...
    if "database" in tech_choice and "orm" in tech_choice:
        database = tech_choice["database"]
        orm = tech_choice["orm"]
        ...
        for db_type in ["sql", "nosql"]:
            for db in TECH_STACK_DATA["techStackOptions"]["database"].get(db_type, []):
                if db["name"] == database:
                    database_data = db
                    break
            if database_data:
