...
class LemonSqueezyService:
    ...
    async def get_subscription_plans(self) -> List[SubscriptionPlan]:
        ...
        try:
            ...
            logger.info(f"Received subscription plans. Products count: {len(response.get('data', []))}, Included items: {len(response.get('included', []))}")
            ...
            if "data" in response:
                for product in response["data"]:
                    ...
                    variant_ids = []
                    if "relationships" in product and "variants" in product["relationships"] and "data" in product["relationships"]["variants"]:
                        variant_ids = [v["id"] for v in product["relationships"]["variants"]["data"]]
                    ...
                    for variant in variants:
                        ...
                        logger.debug(f"Processing variant: {variant['id']} - is_subscription: {variant_attrs.get('is_subscription', False)}")
                        ...
                        if variant_attrs.get("is_subscription", False):
                            ...
                            plan = SubscriptionPlan(
                                ...
                                description=variant_attrs.get("description", "") or product_attrs.get("description", ""),
    ...
    async def get_customer_subscriptions(self, customer_id: str) -> List[Subscription]:
        ...
        try:
            logger.info(f"Getting subscriptions for customer {customer_id} using relationship endpoint")
            ...
            if isinstance(response, dict) and "data" in response:
                ...
                for sub_data in data_items:
                    try:
                        ...
                        subscription = Subscription(
                            id=sub_data["id"],
                            status=status,
                            current_period_end=datetime.fromisoformat(renews_at.replace("Z", "+00:00")) if renews_at else datetime.now(),
                            plan_id=variant_id,
                ...
                if subscriptions:
                    ...
                    logger.warning(f"Empty subscriptions list for customer {customer_id} after processing")
            else:
                # It looks like response format is not as expected or data is empty
                logger.warning(f"No data found in response or empty data array for customer {customer_id}")
        ...
        try:
            ...
            if isinstance(response, dict) and "data" in response:
                    ...
                    try:
                        ...
                        if "relationships" in sub_data and "customer" in sub_data["relationships"]:
                            ...
                            if "data" in customer_rel and "id" in customer_rel["data"]:
                                ...
                                logger.debug(f"Subscription {sub_data['id']} belongs to customer: {sub_customer_id}")
                        ...
                        if belongs_to_customer:
                            logger.info(f"Found matching subscription {sub_data['id']} for customer {customer_id}")
                            
                            if "attributes" not in sub_data:
                                logger.warning(f"Missing attributes in matching subscription: {sub_data}")
                                continue
                ...
                if not subscriptions:
                    ...
                    logger.debug(f"Special case: Searching for customer_id={customer_id} in full response text")
                    response_str = str(response)
                    if f"customer_id': {customer_id}" in response_str or f'"customer_id":{customer_id}' in response_str:
                        logger.warning(f"Customer ID {customer_id} found in response but couldn't extract subscription properly")
            
            logger.info(f"Fallback found {len(subscriptions)} subscriptions for customer {customer_id}")
            return subscriptions
        ...
        try:
            ...
            if isinstance(response, dict) and "data" in response:
                ...
                if "attributes" in sub_data:
                    ...
                    logger.info(f"Found subscription {subscription_id} with status {subscription.status}")
                    return subscription
                else:
                    logger.warning(f"Missing 'attributes' in subscription data: {sub_data}")
