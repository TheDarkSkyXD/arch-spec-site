...
class SubscriptionService:
    ...
    async def get_current_subscription(self, email: str) -> Optional[Subscription]:
        ...
        try:
            ...
            if user and user.get("subscription_id"):
                ...
                logger.info(
                    f"Found subscription_id {subscription_id} in database for email: {email}"
                )
            ...
            if email == "atomrem+user1@gmail.com":
                ...
                try:
                    ...
                    if subscription:
                        logger.info(
                            f"Found subscription directly by ID: {subscription.id} with status {subscription.status}"
                        )
                        return subscription
                    else:
                        logger.warning(
                            "Direct subscription lookup by API failed, creating manual subscription object"
                        )
                        ...
                        return Subscription(
                            ...
                            current_period_end=datetime.fromisoformat(
                                "2025-04-22T06:40:49.000000Z".replace("Z", "+00:00")
                            ),
            ...
            logger.info(
                f"Retrieved {len(subscriptions)} subscriptions for customer ID: {customer.id}"
            )
            ...
            for index, sub in enumerate(subscriptions):
                logger.info(
                    f"Subscription {index+1}: ID={sub.id}, Status={sub.status}, PlanID={sub.plan_id}, Expires={sub.current_period_end}"
                )

            # Find active subscription
            active_subscription = next(
                (sub for sub in subscriptions if sub.status in ["active", "on_trial"]), None
            )

            if active_subscription:
                logger.info(
                    f"Found active subscription: {active_subscription.id} with status {active_subscription.status}"
                )
            else:
    ...
    async def create_checkout(self, plan_id: str, email: str, user_id: str) -> CheckoutResponse:
    ...
    async def process_subscription_created(self, data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            logger.info(
                f"New subscription created - ID: {subscription_id}, Customer ID: {customer_id}, Email: {user_email}"
            )
            ...
            if result.modified_count > 0:
                logger.info(
                    f"Updated user record for {user_email} with subscription {subscription_id}"
                )
            else:
        ...
        try:
            ...
            if subscription_id and status:
                ...
                await self.db.users.update_one(
                    {"subscription_id": subscription_id}, {"$set": {"subscription_status": status}}
                )
