...
class AnthropicDirectClient(BaseLLMClient):
    ...
    def __init__(
        self, llm_logger: Optional[LLMLogger] = None, usage_tracker: Optional[UsageTracker] = None
    ) -> None:
    ...
    def count_tokens(
        ...
        try:
            params: Dict[str, Any] = {"model": model if model else self.model, "messages": messages}
    ...
    async def generate_response(
        ...
        try:
            ...
            # Log the error
            self._log_error(
                error=e, response_type=response_type or "generate_response", metadata=metadata
            )
    ...
    async def get_tool_use_response(
        ...
        # Prepare metadata
        metadata = self._prepare_tool_log_metadata(
            messages, system_prompt, tools, model, log_metadata
        )
        model_to_use = model if model else self.model
        ...
        try:
            ...
            if model_to_use == INTELLIGENT_MODEL:
                ...
                logger.info(
                    f"Params using Claude 3.7 and token-efficient-tools-2025-02-19: {params}"
                )
            ...
            # Log the error
            self._log_error(
                error=e, response_type=response_type or "tool_use_response", metadata=metadata
            )
