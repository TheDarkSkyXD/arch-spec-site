...
class TemplatesService:
        ...
        try:
            ...
            if database is not None:
                ...
                if templates and len(templates) > 0:
                    ...
                    for template in templates:
                        ...
                        complete_template = TemplatesService._ensure_complete_template(
                            template.get("template", template)
                        )

                        # Ensure each template in the response has id and template fields
                        formatted_templates.append(
                            {"id": template_id, "template": complete_template}
                        )
            ...
            for template_data in seed_templates:
                if "template" in template_data:
                    template_data["template"] = TemplatesService._ensure_complete_template(
        ...
        try:
            ...
            if database is not None:
                ...
                if template:
                    ...
                    return {"id": template_id, "template": complete_template}
    ...
    def _ensure_complete_template(template: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "requirements" not in template:
            template["requirements"] = {"functional": [], "non_functional": []}
        ...
        if "pages" not in template:
            template["pages"] = {"public": [], "authenticated": [], "admin": []}

        # Ensure dataModel is present
        if "dataModel" not in template:
            template["dataModel"] = {"entities": [], "relationships": []}
        ...
        if "testing" not in template:
            template["testing"] = {
                "unit": {"framework": "", "coverage": 0},
                "integration": {"framework": "", "coverage": 0},
                "e2e": {"framework": "", "coverage": 0},
            }

        # Ensure projectStructure is present
        if "projectStructure" not in template:
            template["projectStructure"] = {"frontend": {"directories": [], "files": []}}

        # Ensure deployment is present
        if "deployment" not in template:
            template["deployment"] = {"environments": [], "cicd": {"provider": "", "steps": []}}

        # Ensure documentation is present
        if "documentation" not in template:
            template["documentation"] = {"sections": [], "diagrams": []}
