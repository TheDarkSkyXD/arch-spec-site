...
class LLMClientFactory:
    ...
    def create_client(
        ...
        preferred_provider = settings.llm.preferred_provider.lower() if hasattr(settings.llm, 'preferred_provider') else "anthropic"
        enable_failover = settings.llm.enable_failover if hasattr(settings.llm, 'enable_failover') else False
        ...
        if (client is None or client.client is None) and enable_failover:
            logger.warning(f"Preferred LLM provider '{preferred_provider}' failed to initialize, trying alternatives")
