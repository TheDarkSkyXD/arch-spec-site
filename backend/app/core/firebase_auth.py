...
try:
    ...
    if (
        ...
        logger.warning(
            "Firebase environment variables not found. Running in development mode only."
        )
except Exception as e:
...
class FirebaseAuth:
    ...
    async def authenticate_token(self, token: str) -> Dict[str, Any]:
        ...
        if (
            ...
            and settings_local.ENVIRONMENT == "development"
            and not token.startswith("ey")
        ):  # Check if it's not a JWT token
        ...
        try:
            ...
            logger.info(
                f"Successfully verified Firebase token for user: {decoded_token.get('email', 'unknown')}"
            )
        ...
        try:
            ...
            if user:
                ...
                updated_user = await UserService.update_user(
                    str(user["_id"]), {"last_login": datetime.now(UTC)}
                )
                if not updated_user:
    ...
    async def get_test_user(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if test_user:
                # Update last login time
                updated_user = await UserService.update_user(
                    str(test_user["_id"]), {"last_login": datetime.now(UTC)}
                )
                if not updated_user:
