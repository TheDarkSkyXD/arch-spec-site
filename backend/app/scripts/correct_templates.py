...
def correct_template_data() -> List[Dict]:
    ...
    for i, template in enumerate(corrected_templates):
        ...
        if (
            ...
            and isinstance(tech_stack["frontend"], dict)
            and "frameworks" in tech_stack["frontend"]
        ):
            ...
            for j, framework in enumerate(tech_stack["frontend"].get("frameworks", [])):
                ...
                if name and not utils.is_valid_tech(name):
                    ...
                    if closest_match:
                        ...
                        corrected_templates[i]["template"]["techStack"]["frontend"]["frameworks"][
                            j
                        ]["name"] = closest_match
                        corrections_made += 1
                ...
                for compat_category, compat_techs in framework.get("compatibility", {}).items():
                    for k, tech in enumerate(compat_techs):
                        if not utils.is_valid_tech(tech):
                            ...
                            if closest_match:
                                logger.info(
                                    f"Correcting compatibility tech: {tech} -> {closest_match}"
                                )
                                corrected_templates[i]["template"]["techStack"]["frontend"][
                                    "frameworks"
                                ][j]["compatibility"][compat_category][k] = closest_match
                                corrections_made += 1
            ...
            for j, framework in enumerate(tech_stack.get("backend", {}).get("frameworks", [])):
                ...
                if name and not utils.is_valid_tech(name):
                    ...
                    if closest_match:
                        ...
                        corrected_templates[i]["template"]["techStack"]["backend"]["frameworks"][j][
                            "name"
                        ] = closest_match
                        corrections_made += 1
                    ...
                    for k, tech in enumerate(compat_techs):
                        if not utils.is_valid_tech(tech):
                            ...
                            if closest_match:
                                ...
                                corrected_templates[i]["template"]["techStack"]["backend"][
                                    "frameworks"
                                ][j]["compatibility"][compat_category][k] = closest_match
                                corrections_made += 1
            ...
            for j, baas in enumerate(tech_stack.get("backend", {}).get("baas", [])):
                ...
                if name and not utils.is_valid_tech(name):
                    ...
                    if closest_match:
                        ...
                        corrected_templates[i]["template"]["techStack"]["backend"]["baas"][j][
                            "name"
                        ] = closest_match
                        corrections_made += 1
                    ...
                    for k, tech in enumerate(compat_techs):
                        if not utils.is_valid_tech(tech):
                            ...
                            if closest_match:
                                ...
                                corrected_templates[i]["template"]["techStack"]["backend"]["baas"][
                                    j
                                ]["compatibility"][compat_category][k] = closest_match
                                corrections_made += 1
                ...
                for j, db in enumerate(tech_stack.get("database", {}).get(db_type, [])):
                    ...
                    if name and not utils.is_valid_tech(name):
                        ...
                        if closest_match:
                            ...
                            corrected_templates[i]["template"]["techStack"]["database"][db_type][j][
                                "name"
                            ] = closest_match
                            corrections_made += 1
                        ...
                        for k, tech in enumerate(compat_techs):
                            if not utils.is_valid_tech(tech):
                                ...
                                if closest_match:
                                    ...
                                    corrected_templates[i]["template"]["techStack"]["database"][
                                        db_type
                                    ][j]["compatibility"][compat_category][k] = closest_match
                                    corrections_made += 1
                ...
                for j, value in enumerate(value_list):
                    if not utils.is_valid_tech(value):
                        ...
                        if closest_match:
                            ...
                            corrected_templates[i]["template"]["techStack"]["hosting"][key][
                                j
                            ] = closest_match
                            corrections_made += 1
            ...
            for key, value_list in tech_stack.get("authentication", {}).items():
                for j, value in enumerate(value_list):
                    if not utils.is_valid_tech(value):
                        ...
                        if closest_match:
                            logger.info(
                                f"Correcting authentication option: {value} -> {closest_match}"
                            )
                            corrected_templates[i]["template"]["techStack"]["authentication"][key][
                                j
                            ] = closest_match
                            corrections_made += 1
        else:
            ...
            if "frontend" in tech_stack:
                ...
                for key, value in frontend.items():
                    if (
                        ...
                        and not utils.is_valid_tech(value)
                    ):
                        closest_match = find_closest_match(value)
                        if closest_match:
                        ...
                        for j, option in enumerate(value):
                            if not utils.is_valid_tech(option):
                                ...
                                if closest_match:
                                    logger.info(
                                        f"Correcting frontend option: {option} -> {closest_match}"
                                    )
                                    corrected_templates[i]["template"]["techStack"]["frontend"][
                                        "options"
                                    ][j] = closest_match
