...
vi.mock("../PreviewFactory", () => ({
  default: vi.fn(() => (
    <div data-testid="preview-factory">Mocked Preview Factory</div>
  )),
}));
...
describe("UIDesignPreview", () => {
  ...
  it("should render empty state when no data is provided", () => {
    ...
    expect(
      screen.getByText("No UI design data available to preview")
    ).toBeInTheDocument();
  });
  ...
  it("should render preview with data correctly", () => {
    ...
    expect(markdownService.generateUIDesignMarkdown).toHaveBeenCalledWith(
      mockUIDesign
    );
