...
jest.mock("../PreviewFactory", () => {
  return jest.fn(() => (
    <div data-testid="preview-factory">Preview Factory</div>
  ));
});
...
describe("RequirementsPreview", () => {
  ...
  beforeEach(() => {
    ...
    (markdownService.generateFileName as jest.Mock).mockReturnValue(
      "test-project-requirements.md"
    );
  });
  ...
  test("should render loading state when isLoading is true", () => {
    ...
    expect(
      screen.getByRole("article").querySelector(".animate-pulse")
    ).toBeInTheDocument();
  });
  ...
  test("should generate markdown and render PreviewFactory when data is available", () => {
    ...
    expect(markdownService.generateRequirementsMarkdown).toHaveBeenCalledWith(
      mockRequirementsData
    );
    expect(markdownService.generateFileName).toHaveBeenCalledWith(
      mockProjectName,
      "requirements"
    );
