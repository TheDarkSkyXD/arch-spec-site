...
const Select = ({
  ...
  return (
    <SelectContext.Provider value={{ open, setOpen, value, setValue, onValueChange }}>
      <div className={cn('relative w-full', disabled && 'pointer-events-none opacity-50')}>
        {children}
      </div>
  ...
  ({ className, children, value: itemValue, disabled, ...props }, ref) => {
    ...
    return (
      ...
      >
        ...
        {isSelected && <Check className="ml-2 h-4 w-4 text-primary-600 dark:text-primary-400" />}
      </div>
    );
  }
);
