...
const BackendSection = ({
  ...
  const filteredRealtime = useMemo(
    ...
    [backendType, backendFramework, backendService, backendFrameworks, backendRealtime]
  );

  const filteredBaaS = useMemo(
    () => filterBackendBaaSOptions(backendRealtimeValue, backendFunctionsValue, backendBaaS),
  ...
  useEffect(() => {
    if (!initialData || initialDataAppliedRef.current || !initialData.backend) return;

    console.log('Checking initial data for backend section:', initialData.backend);
    ...
    if (backend.type === 'framework') {
      ...
      const frameworkExists = backendFrameworks.some((fw) => fw.id === frameworkBackend.framework);

      if (frameworkExists) {
        setValue('backend_framework', frameworkBackend.framework, {
          shouldDirty: true,
        });
        console.log('Setting initial backend framework:', frameworkBackend.framework);
      } else {
        console.log(
      ...
      if (frameworkBackend.realtime) {
        const realtimeExists = backendRealtime.some((rt) => rt.id === frameworkBackend.realtime);

        if (realtimeExists) {
          setValue('backend_realtime', frameworkBackend.realtime, {
            shouldDirty: true,
          });
          console.log('Setting initial realtime support:', frameworkBackend.realtime);
        }
      }
      ...
      const serviceExists = backendBaaS.some((baas) => baas.id === baasBackend.service);
      ...
      if (serviceExists) {
        ...
        console.log('Initial BaaS service not available in options:', baasBackend.service);
      }

      // Set functions if provided
      if (baasBackend.functions) {
        const functionsExists = backendServerless.some((func) => func.id === baasBackend.functions);

        if (functionsExists) {
          setValue('backend_functions', baasBackend.functions, {
            shouldDirty: true,
          });
          console.log('Setting initial serverless functions:', baasBackend.functions);
        }
      }

      // Set realtime if provided
      if (baasBackend.realtime) {
        const realtimeExists = backendRealtime.some((rt) => rt.id === baasBackend.realtime);

        if (realtimeExists) {
          setValue('backend_realtime', baasBackend.realtime, {
            shouldDirty: true,
          });
          console.log('Setting initial realtime support:', baasBackend.realtime);
        }
      }
      ...
      if (serviceExists) {
        ...
        console.log('Setting initial serverless service:', serverlessBackend.service);
      } else {
        console.log(
  ...
  }, [initialData, backendFrameworks, backendBaaS, backendRealtime, backendServerless, setValue]);

  return (
    <div>
      <h3 className="mb-4 text-lg font-medium text-slate-800 dark:text-slate-100">Backend</h3>
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
        ...
        {backendType === 'framework' && (
          <>
            ...
            <div>
              <Label htmlFor="backend_realtime">Realtime Support (Optional)</Label>
        ...
        {backendType === 'baas' && (
          <>
            ...
            <div>
              <Label htmlFor="backend_functions">Serverless Functions (Optional)</Label>
