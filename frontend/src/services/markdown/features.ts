 ...
 */
export function generateFeaturesMarkdown(data: Partial<FeaturesData> | null): string {
  if (!data) return '';

  let markdown = `# Project Features\n\n`;

  // Helper function to render feature sections
  const renderFeatureSection = (title: string, features: FeatureModule[] | undefined) => {
    ...
    features.forEach((feature: FeatureModule, index: number) => {
      ...
      section += `### ${index + 1}. ${feature.name} (${statusLabel}, ${optionalLabel})\n\n`;
