    ...
    .map(([key, value]) => {
      ...
      if (typeof value === 'object' && value !== null) {
        if (Array.isArray(value)) {
          ...
          const nestedProps = formatObject(value as Record<string, unknown>, indentLevel + 1);
          if (nestedProps.trim() === '') {
 ...
 */
export function generateFileName(projectName: string | undefined, section: string): string {
  const sanitizedName = (projectName || 'project').toLowerCase().replace(/[^a-z0-9]/g, '-');
