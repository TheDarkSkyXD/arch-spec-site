...
describe("TechStackForm functionality", () => {
  ...
  it("should convert form data to API format correctly", async () => {
    ...
    vi.mocked(techStackService.saveTechStack).mockResolvedValueOnce(
      expectedApiData
    );

    // Call the service with mock data
    const result = await techStackService.saveTechStack(
      mockProjectId,
      mockFormData
    );

    // Verify service was called with correct data
    expect(techStackService.saveTechStack).toHaveBeenCalledWith(
      mockProjectId,
      mockFormData
    );
  ...
  it("should handle errors when saving tech stack", async () => {
    ...
    vi.mocked(techStackService.saveTechStack).mockRejectedValueOnce(
      new Error("API error")
    );

    // Call and verify error is thrown
    await expect(
      techStackService.saveTechStack(mockProjectId, mockFormData)
